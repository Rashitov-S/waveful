...
from database import UserModel, AlbumModel, ArtistModel, TrackModel, FavouriteModel
...
class AddTrackDialog(AddTrackDialogUI):
    ...
    def accept_dialog(self):
        try:
            ...
            destination_album_path = self.copy_album_image() if self.album_image else None
            ...
            self.track_model.add(title, artist_id, album_id, destination_music_path)
            self.accept()
    ...
    def get_artist_id(self):
        if not self.art_combo:
            return self.artist_model.get(self.artist_input.text())[0][0]
    ...
    def copy_music_file(self):
        if self.music_file:
            _, extension = os.path.splitext(self.music_file)
            destination_folder_music = 'resources\\tracks'
            track_id = self.track_model.get_max_id()[0] if self.track_model.get_max_id()[0] else 0
            new_file_path = os.path.join(destination_folder_music,
                                         str(track_id + 1) + f"{extension}")
            os.makedirs(destination_folder_music, exist_ok=True)
            return shutil.copy(self.music_file, new_file_path)
        return None

    def copy_album_image(self):
        if self.album_image:
            destination_folder_album = 'resources\\album_covers'
            os.makedirs(destination_folder_album, exist_ok=True)
            return shutil.copy(self.album_image, destination_folder_album)
        return None

    def get_album_id(self, album_name):
        return self.album_model.get_id(album_name)[0] if not self.alb_combo else self.album_model.get_id(
    ...
    def add_album(self, path):
        ...
        if self.art_combo:
            ...
            artist_id = self.artist_model.get(self.artist_input.text())[0][0]
        if title:
            self.album_model.add(title, artist_id, path)
    ...
    def update_artist_box(self):
        ...
        artists = self.artist_model.get(False)
    ...
    def update_album_box(self):
        ...
        artist_id = self.artist_combobox.currentIndex() + 1
        albums = self.album_model.get(artist_id)
        for album in albums:
...
class MainWindow(MainFormUI):
    ...
    def like(self):
        if not self.favourite_model.get_track(self.session_id, self.current_track_id):
            self.favourite_model.add(self.session_id, self.current_track_id)
            self.status_bar.add_favourite_button.change_icon("resources\\icons\\liked_icon.png",
                                                             "resources\\icons\\liked_icon.png")
        else:
            self.favourite_model.delete(self.session_id, self.current_track_id)
    ...
    def play(self, id):
        track_info = self.track_model.get(id)[0]
        artist_info = self.artist_model.get_name(track_info[2])
        album_path = self.album_model.get_image(track_info[3])
        self.media_player.setSource(QUrl.fromLocalFile(track_info[4]))
        self.current_track_id = id
        self.media_player.play()
        self.status_bar.display(track_info[1], artist_info[0], album_path[0], get_track_length(track_info[4]),
...
class LoginWindow(LoginFormUI):
    ...
    def register(self):
        ...
        try:
            ...
            if not user:
                self.user_model.add(login, password)
    ...
    def is_user_exist(self, login):
        ...
        user = self.user_model.get(login)
