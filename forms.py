...
from database import AlbumModel, ArtistModel, TrackModel, FavouriteModel, UserModel
...
class PlaylistTable(QTableWidget):
    ...
    def update_table(self):
        self.tracks_id = []
        self.setRowCount(0)
        tracks = self.track_model.get()
        self.tracks = tracks
        num = 1
        for track in tracks:
            self.tracks_id.append(track[0])
            duration = get_track_length(track[4])
            self.add_track(str(num), track[1], self.artist_model.get_name(track[2])[0],
                           self.album_model.get_title(track[3])[0], duration, self.album_model.get_image(track[3])[0])
...
class FavouritePlaylistTable(PlaylistTable):
    ...
    def update_table(self):
        self.tracks_id = []
        self.setRowCount(0)
        favourite_tracks = self.favourite_model.get_tracks(self.user_id)
        tracks = []
        for id in favourite_tracks:
            tracks.extend(self.track_model.get(id[0]))
        self.tracks = tracks
        num = 1
        for track in tracks:
            self.tracks_id.append(track[0])
...
class SearchPlaylistTable(PlaylistTable):
    ...
    def update_table(self, title=None):
        if title:
            self.tracks_id = []
            self.setRowCount(0)
            search_tracks = self.track_model.search(title)
            ...
            for track in search_tracks:
                ...
                duration = get_track_length(track[4])
                self.add_track(str(num), track[1], self.artist_model.get_name(track[2])[0],
                               self.album_model.get_title(track[3])[0], duration,
                               self.album_model.get_image(track[3])[0])
                self.setRowHeight(num - 1, 40)
                num += 1
            fnt = self.font()
            fnt.setPointSize(11)
...
class PlayStatusBar(QStatusBar):
    ...
    def display(self, title, artist, album_path, duration, track_id, session):
        ...
        if self.favourite_model.get_track(session, track_id):
...
class ProfileContentWidgetUI(QWidget):
    ...
    def change_user_password(self):
        ...
        if new_password:
            self.user_model.change_password(self.user_id, new_password)
        self.set_login_password()
        self.show_pass = True
    ...
    def set_login_password(self):
        self.user = self.user_model.get_by_id(self.user_id)
